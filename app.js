...
angular.module('spendPlan',
  ...
  .filter('active', function() {
	...
	return input;
      ...
      return Object.values(input).filter(function(a) {
	return a.fire.active === undefined || a.fire.active
      });
    }
  })
  .controller(
  ...
  .controller(
    ...
    ['$scope',
	 ...
	 start: moment().subtract(1, 'months').startOf('month'),
       ...
       $scope.lastQtr = {
	 start: moment().subtract(3, 'months').startOf('quarter'),
	 end: moment().subtract(3, 'months').endOf('quarter')
       };
       $scope.lastYr = {
  ...
  .filter('transactions', function() {
    return function(input, search) {
      var filtered = input;
      if (search.acct) {
	filtered = _.filter(filtered, function(val) {
	  var acctRe = new RegExp(search.acct, 'gi');
	  return acctRe.test(val._acct.name);
	});
      }
      if (search.cat) {
	filtered = _.filter(filtered, function(val) {
	  var catRe = new RegExp(search.cat, 'gi');
	  return catRe.test(val.Category);
	});
      }
      if (search.note) {
	filtered = _.filter(filtered, function(val) {
	  var noteRe = new RegExp(search.note, 'gi');
	  return noteRe.test(val.Note);
	});
      }
      if (search.min) {
	filtered = _.filter(filtered, function(val) {
	  var minAmt = Number(search.min);
	  return val._amountUSD >= minAmt;
	})
      }
      if (search.max) {
	filtered = _.filter(filtered, function(val) {
	  var maxAmt = Number(search.max);
	  return val._amountUSD <= maxAmt;
	})
      }
      return filtered;
    }
  ...
  .controller(
    ...
    ['$scope', '$filter', '$mdEditDialog',
	 ...
	 $scope.tx.$save(t);
       ...
       $scope.editCatOnBlur = function(t, cat) {
	 t.Category = cat;
	 $scope.tx.$save(t);
       };
	 ...
	 // });
       ...
       $scope.editTags = function(event, t) {
	 var editDialog = {
	   modelValue: (t.Tags || []).join(', '),
	   placeholder: 'Add comma-separated tags',
	   save: function(input) {
	     var tags = input.$modelValue.split(',');
	     for (var idx in tags) {
