...
angular.module('spControllers',['spServices','spFilters','spHelpers','spDirectives']).controller(
    'SpendPlanCtrl', 
    ['$scope','$timeout','$http', '$rootScope',
     'dictValFilter','orderByFilter','transLimiter','spRecordService',
	...
	$scope.showSplits = function(transaction) {
	 ...
	 $scope.splitStatus = function(transaction) {
	     var splitclass = Object.keys(transaction.splits).length?'primary':'default';
	     return 'label label-' + splitclass;
	 }

	$scope.addSplit = function(transaction) {
	    if ($scope.edit_splits.add_cat) {
		transaction.splits[$scope.edit_splits.add_cat] = {amount: $scope.edit_splits.add_amt};
		$scope.edit_splits.add_cat = "";
		$scope.edit_splits.add_amt = 0;
	    };
	};
	...
	$scope.saveSplits = function(transaction) {
		...
		trans_splits.push(Number(val.amount));
	    });
	};
